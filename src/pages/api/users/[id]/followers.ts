 ...
 * 用户粉丝列表接口
 *
 * GET /api/users/:id/followers?page=1&pageSize=20
 * 无需登录鉴权
 */

/** 粉丝记录（含粉丝用户的精简信息，由仓库 include 注入） */
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<PaginatedData<FollowerItem>>>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const { list, total } = await FollowRepository.findFollowers(userId, page, pageSize)

    const items: FollowerItem[] = (list as unknown as FollowWithFollower[]).map((item) => ({
      id: item.follower.id,
    ...
    const message = error instanceof Error ? error.message : '获取粉丝列表失败'
